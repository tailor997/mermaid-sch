...
import { drawPolyline, drawLine, drawText } from '../elements/index.js';
...
export const drawResistor = (parent: D3Element, props: ComponentProps) => {
  ...
  // Dimensions
  // const segmentLen = size / 4; // Length of lead wires
  const zigZagLen = size / 2; // Length of the zig-zag part
  const height = 10; // Height of zig-zag

  // Draw leads
  drawLine(group, { x: -size / 2, y: 0 }, { x: -size / 4, y: 0 }, { stroke: color });
  drawLine(group, { x: size / 4, y: 0 }, { x: size / 2, y: 0 }, { stroke: color });
